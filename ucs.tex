...
\section*{The Universal Coordinate System}

\subsection*{Definitions and Fundamental Constants}

Let the following be the fundamental constants and definitions in the universal coordinate system:
...
\begin{equation}
d_\text{BAO} = \frac{r_d^{\text{LCDM}} \left(1 - \gamma t_\text{drag}\right)}{\theta_\text{BAO}(z)}
\end{equation}
...
\subsection*{Calculating Distance in Light-Years}

To convert the Cartesian coordinates $(X, Y, Z)$ to a distance in light-years, we can use the following formula:

\begin{equation}
d_\text{ly} = \frac{\sqrt{X^2 + Y^2 + Z^2}}{0.306609}
\end{equation}

where $d_\text{ly}$ is the distance in light-years.

The factor 0.306609 is the conversion factor from parsecs to light-years, where 1 parsec $\approx$ 3.26156 light-years.

By applying this formula to the Cartesian coordinates of any two points in the universal coordinate system, we can calculate the distance between them in light-years.
